...
if __name__=='__main__':
    ...
    max_sampleX = sampleX.max()
    max_sampleY = sampleY.max()
    # num_size = round(sampleX1.size * 0.5)
    increases_X = random.sample(range(1, 100), 6)
    increases_Y = random.sample(range(20, 80), 6)
    for increase_x in increases_X:
        increases_sampleX = max_sampleX * (float(increase_x) / 100)
        sampleX = np.row_stack((sampleX, increases_sampleX))
    for increase_y in increases_Y:
        increases_sampleY = max_sampleY * (float(increase_y) / 100)+max_sampleY
        sampleY = np.row_stack((sampleY, increases_sampleY))
    korder=5
    # least_squares(LS)·
